...
parser.add_argument('--save_name', default='train')
parser.add_argument('--datapath', default='./datasets/DUTS_256',help='dataset_path')
parser.add_argument('--mode', default='end2end',help='end2end or decisionmap, only end2end can be used now')
...
train_left_img, train_right_img, test_left_img, test_right_img, label_train_fusion_img,label_val_fusion_img = Dataloader.dataloader(args.datapath,mode=args.mode)

TrainImgLoader = torch.utils.data.DataLoader(
         Dataloader.myImageFloder(train_left_img,train_right_img,label_train_fusion_img,augment=False,mode=args.mode),
         batch_size= args.batch_size, shuffle= True, num_workers=8,pin_memory=True)

TestImgLoader = torch.utils.data.DataLoader(
        Dataloader.myImageFloder(test_left_img,test_right_img,label_val_fusion_img,augment=False,mode=args.mode),
...
#define models
model = SwinMFF_Net(img_size=to_2tuple(args.input_size),depths=[2, 2, 2, 6, 2, 2],num_heads=[6,6,6,6,6,6])
...
def test(imgL, imgR, img_label,mode='end2end'):
...
def main(args):
    ...
    for epoch in tqdm(range(start_epoch, args.epochs)):
        ...
        for batch_idx, (imgL, imgR, img_label) in enumerate(tqdm_bar_train):
            train_loss = train(imgL, imgR, img_label,mode=args.mode)
        ...
        for batch_idx, (imgL, imgR, img_label) in enumerate(tqdm_bar_val):
            test_loss = test(imgL, imgR, img_label,mode=args.mode)
